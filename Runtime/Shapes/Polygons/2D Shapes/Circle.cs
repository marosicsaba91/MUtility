...
{
	[Serializable]
	public struct Circle : IPolygon, IWireShape, IEasyHandleable, IArea
	{
		...
		public WireShape ToWireShape() => new(Points.ToArray());

		public WireShape ToDrawable(int segmentCount, bool dashedLine = false)
		{
			if (dashedLine)
				return ToDrawable_Dashed(segmentCount);
			return new WireShape(ToPolygon(segmentCount).ToArray());
		}

		public WireShape ToDrawable_Dashed(int segmentCount = defaultSegmentCount)
		{
			segmentCount /= 2;
			Vector2 right = new(radius, 0);
			Vector2 up = new(0, radius);

			Vector3[][] polygons = new Vector3[segmentCount][];
