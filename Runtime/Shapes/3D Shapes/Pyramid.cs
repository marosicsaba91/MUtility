...
{
	[Serializable]
	public struct Pyramid : IWireShape
	{
		...
		public LineSegment BalseLine => new(basePos, Apex);
		...
		public WireShape Slice(Plain plain) => CubeSliceHelper.Slice(plain,
			FrustumLeftBottom,
			BaseLeftBottom,
			FrustumLeftTop,
			BaseLeftTop,
		...
		public WireShape ToWireShape()
		{
			Vector3 ap = new(0, fullHeight, 0);

			Vector3 p1 = new(baseRect.x / 2f, 0, baseRect.y / 2f);
			Vector3 p2 = new(baseRect.x / 2f, 0, -baseRect.y / 2f);
			Vector3 p3 = new(-baseRect.x / 2f, 0, baseRect.y / 2f);
			Vector3 p4 = new(-baseRect.x / 2f, 0, -baseRect.y / 2f);
