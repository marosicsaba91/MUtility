...
namespace MUtility
{
	public readonly struct WireShape
	{
		...
		public WireShape(params Vector3[][] polygons)
		{
		...
		public void Merge(WireShape other)
		{
		...
		public WireShape GetTransformed(Transform transform) =>
			CreateCopy(v3 => transform.TransformPoint(v3));

		public WireShape GetTranslated(Vector3 offset) =>
			CreateCopy(v3 => offset + v3);

		public WireShape GetRotated(Quaternion rotate) =>
			CreateCopy(v3 => rotate * v3);


		public WireShape GetScaled(float scale) =>
			CreateCopy(v3 => scale * v3);

		public WireShape GetScaled(Vector3 scale) =>
			CreateCopy(v3 => v3.MultiplyAllAxis(scale));

		public WireShape GetTransformed(Vector3 offset, Quaternion rotate, float scale) =>
			CreateCopy(v3 => (rotate * (offset + v3)) * scale);

		public WireShape GetTransformed(Vector3 offset, Quaternion rotate, Vector3 scale) =>
			CreateCopy(v3 => (rotate * (offset + v3)).MultiplyAllAxis(scale));

		// ------------------------------------------------------------------

		public WireShape To3D(Axis3D normalAxis)
		{
