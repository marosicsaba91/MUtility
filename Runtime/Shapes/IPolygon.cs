...
namespace MUtility
{
	...
	{
		public static WireShape ToDrawable(this IEnumerable<Vector3> polygon)
		{
			Vector3[] points = polygon.ToArray();
			return new WireShape(points);
		}
		...
		public static void DrawGizmo(this IEnumerable<Vector3> polygon, Color color) =>
			DrawPolygon(polygon, color, WireShape.DrawingType.Gizmo);

		public static void DrawGizmo(this IEnumerable<Vector3> polygon) =>
			DrawPolygon(polygon, default, WireShape.DrawingType.Gizmo);


		public static void DrawDebug(this IEnumerable<Vector3> polygon, Color color) =>
			DrawPolygon(polygon, color, WireShape.DrawingType.Debug);

		public static void DrawDebug(this IEnumerable<Vector3> polygon) =>
			DrawPolygon(polygon, default, WireShape.DrawingType.Debug);


		public static void DrawHandle(this IEnumerable<Vector3> polygon, Color color) =>
			DrawPolygon(polygon, color, WireShape.DrawingType.Handle);

		public static void DrawHandle(this IEnumerable<Vector3> polygon) =>
			DrawPolygon(polygon, default, WireShape.DrawingType.Handle);


		// Private

		static void DrawPolygon(IEnumerable<Vector3> polygon, Color color, WireShape.DrawingType type)
		{
			if (type == WireShape.DrawingType.Debug)
			{
				Vector3? last = null;
			...
			else if (type == WireShape.DrawingType.Gizmo)
			{
				if (color != default)
					Gizmos.color = color;
			...
			else if (type == WireShape.DrawingType.Handle)
			{
#if UNITY_EDITOR
