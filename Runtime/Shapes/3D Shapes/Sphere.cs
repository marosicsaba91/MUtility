...
namespace MUtility
{
	...
	public struct Sphere : IMesh/*, IEasyHandleable*/
	{
		...
		*/
		...
		public WireShape ToDrawable() => ToDrawable(defaultGizmoComplexity, Circle.defaultSegmentCount);

		public WireShape ToDrawable(int complexity, int circleSegmentCount)
		{
			Vector3[][] polygons = new Vector3[complexity * 2][];
			Circle unitCircle = new(radius);
			for (int i = 0; i < complexity; i++)
			{
				// Width
				float h = -radius + (1 + i) * (2 * radius) / (complexity + 1);
				float circleRadius = Mathf.Sqrt(radius * radius - h * h);
				Circle c1 = new(circleRadius);
