...
{
    readonly CurveEditorPreview _curvePreview = new CurveEditorPreview();
    ...
    public override bool Draw(
        ...
        GUIContent label)
    {
        var curve = (IInspectorCurve) inspectorElement;
        ...
        if (GUI.Button(foldoutPosition, string.Empty, foldoutButtonStyle))
            property.isExpanded = !property.isExpanded;

        var previewPosition = new Rect(
            position.x + EditorGUIUtility.labelWidth + 3,
            position.y,
            position.width - EditorGUIUtility.labelWidth - 3,
            position.height);

        float Func(float t) => curve.Evaluate(parentObject, t);
        _curvePreview.Draw(
            previewPosition,
            Func,
            curve.DefaultArea(parentObject),
            curve.GetCurveColor(parentObject),
            property.isExpanded);
        return false;
    }
        ...
        GUIContent label)
    {
        ...
        var curve = (IInspectorCurve) property.GetObjectOfProperty();
        return curve.Height(parentObject);
    }
}
